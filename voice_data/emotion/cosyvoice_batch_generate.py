...
url = "http://172.31.208.3:50000/api/inference/sft"
...
# 音色选项
voices = {
    "音色1": {"seed": 1111},  "中文女": {"age":"成年", "sex": "female"}, 
    "音色2": {"seed": 2222},  "中文男": {"age":"成年", "sex": "male"}, 
...
def save_voice(data_zip,out_dir,gpt_gene):
  ...
  res_questions = requests.request("POST", url, headers=headers, data=questions,stream=True,timeout=600)
  # 暂停三秒
  # time.sleep(3)
  res_answers = requests.request("POST", url, headers=headers, data=answers,stream=True,timeout=600)
  # 暂停三秒
  # time.sleep(3)
  ...
  if res_questions.status_code == 200 and res_answers.status_code == 200 :
    ...
    try:
        ...
        que_filename=f"{timestamp}_queseed{questions['role']}"
        ans_filename=f"{timestamp}_ansseed{answers['role']}"
        ...
        result={
                  ...
                  "input": {
                    ...
                    "language": "zh",
                    "seed":questions['role'],
                    "transcription": questions['tts'],  #我今天真的很开心，因为我通过了考试。text内容
                    "emotion": gpt_gene['emotion'],   #情绪 开心
                    "gender": voices[str(questions['role'])]['sex'],    #female
                    "age": voices[str(questions['role'])]['age'],         #成人
                    "speed": "",       #正常
                    "volume": "",      #中等
                    "tone": ""         #愉快
                  },
                  "output": {
                    ...
                    "language": "zh",
                    "seed":answers['role'],
                    "transcription": answers['tts'], #太好了，祝贺你！我们应该好好庆祝一下。
                    "emotion": gpt_gene['emotion'],       #兴奋
                    "gender": voices[str(answers['role'])]['sex'],        #male
                    "age": voices[str(answers['role'])]['age'],           #成人
                    "speed": "",         #正常
                    "volume": "",        #中等
                    "tone": ""           #明亮
                  }
...
if __name__ == '__main__':
  # data_dir='./out_use/Gpt4_emo' #数据集路径
  data_dir='./text_out/Gpt4_emo' #文本数据集路径 czc_mtt/dataset_create/voice_data/emotion/voice_out/cosyvoice_emo

  out_dir='voice_out/cosyvoice_emo'  #输出的路径
